#!/usr/bin/env python3
"""
Monitor for FastExactScraper progress
"""
import os
import time
import glob
...
def is_running():
    try:
        ...
        return 'fast_exact_scraper.py' in out.stdout
    except Exception:
        return False

def latest_progress():
    files = glob.glob('players_FAST_EXACT_PROGRESS_*.csv')
    if not files:
        return None
...
def show():
    clear_screen()
    print("⚡ FAST EXACT SCRAPER MONITOR")
    print("="*60)
    ...
    lp = latest_progress()
    if not lp:
        print("⏳ Waiting for first progress file (saved every 50 players)...")
        return
    fname, df = lp
