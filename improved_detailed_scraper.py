#!/usr/bin/env python3
"""
Improved detailed player profile scraper with better selectors for Transfermarkt
"""
import requests
import pandas as pd
import time
...
class ImprovedDetailedPlayerScraper:
    def __init__(self):
    ...
    def extract_player_details(self, player_url, basic_info):
        ...
        try:
            # Market value (current) - Look in the header area
    ...
    def _extract_market_value(self, soup):
        ...
        try:
            # Look for market value in the header area
            value_selectors = [
                '.dataMarktwert a',
                '.dataMarktwert',
                '.tm-player-market-value-development__current-value',
                '.dataMarktwert .dataValue',
                '.dataMarktwert .dataValue a',
                '.tm-player-header__market-value',
                '.dataMarktwert .dataValue span'
            ]
    ...
    def _extract_date_of_birth(self, soup):
        ...
        try:
            # Look for date of birth in the data table
            dob_selectors = [
                '.dataGeburtsdatum',
                '.dataGeburtsdatum .dataValue',
                '.dataGeburtsdatum .dataValue a',
                '.tm-player-header__birth-date',
                '.dataGeburtsdatum span'
            ]
    ...
    def _extract_nationality(self, soup):
        ...
        try:
            ...
            nationality_selectors = [
                ...
                '.dataNationalitaet .dataValue a',
                '.tm-player-header__nationality img',
                '.dataNationalitaet span'
            ]
    ...
    def _extract_position(self, soup):
        ...
        try:
            position_selectors = [
                ...
                '.dataRueckennummer .dataValue a',
                '.tm-player-header__position',
                '.dataRueckennummer span'
            ]
    ...
    def _extract_current_club(self, soup):
        ...
        try:
            club_selectors = [
                ...
                '.dataZugehoerigkeit .dataValue a',
                '.tm-player-header__current-club',
                '.dataZugehoerigkeit span'
            ]
    ...
    def process_players(self, players_df, max_players=20):
...
def main():
    ...
    # Initialize scraper
    scraper = ImprovedDetailedPlayerScraper()
    
    # Process first 20 players for testing
    print(f"\nProcessing first 20 players for detailed information...")
    detailed_players = scraper.process_players(players_df, max_players=20)
    
    if detailed_players:
        # Save detailed data
        df_detailed = pd.DataFrame(detailed_players)
        csv_file = 'players_IMPROVED_DETAILED.csv'
