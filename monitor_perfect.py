#!/usr/bin/env python3
"""
Monitor for the perfect scraper with real-time progress
"""
import os
import time
import glob
...
def get_scraper_status():
    ...
    try:
        ...
        return 'perfect_scraper.py' in result.stdout
    except:
        return False
...
def get_log_tail():
    ...
    try:
        result = subprocess.run(['tail', '-10', 'perfect_scraper.log'], capture_output=True, text=True)
...
def display_progress():
    ...
    if progress:
        ...
        print("-" * 40)
        df = progress['data']
        key_fields = ['age', 'nationality', 'position', 'market_value_str', 'market_value_numeric', 'dominant_foot', 'agent']
        ...
        print("-" * 40)
        sample_cols = ['full_name', 'team_name', 'age', 'nationality', 'position', 'market_value_str', 'market_value_numeric']
        ...
        if available_cols:
            ...
            for _, player in recent_players.iterrows():
                ...
                value_str = player.get('market_value_str', 'N/A')
                value_num = player.get('market_value_numeric', 'N/A')
                if value_num != 'N/A':
                    value_num = f"{value_num:,}"
                print(f"  • {name} ({age}) - {team} - {position} - {value_str} ({value_num})")
...
def main():
    ...
    try:
        ...
        print("📁 Check 'perfect_scraper.log' for detailed logs.")
