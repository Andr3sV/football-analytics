#!/usr/bin/env python3
"""
Detailed player profile scraper - extracts comprehensive data from individual player pages
"""
import requests
import pandas as pd
import time
...
class DetailedPlayerScraper:
        ...
        try:
            ...
            for selector in value_selectors:
                ...
                if value_elem:
                    ...
                    if '€' in value_text or 'mio' in value_text.lower():
                        return value_text
        except:
            pass
        return ""
    ...
    def _calculate_age(self, date_of_birth):
        ...
        try:
            ...
            date_formats = ['%b %d, %Y', '%d.%m.%Y', '%Y-%m-%d', '%B %d, %Y']
    ...
    def process_players(self, players_df, max_players=None):
        """Process players to extract detailed information"""
        print(f"=== Processing {len(players_df)} players for detailed information ===")
        
        if max_players:
            players_df = players_df.head(max_players)
            print(f"Limited to first {max_players} players for testing")
        
        detailed_players = []
        
        for idx, player in players_df.iterrows():
            print(f"\nPlayer {idx + 1}/{len(players_df)}: {player.get('full_name', 'Unknown')}")
...
def main():
    ...
    # Ask user for confirmation and limits
    print(f"\nThis will process {len(players_df)} players to extract detailed information.")
    print("This will take a VERY long time due to delays between requests.")
    
    max_players = input("Enter max number of players to process (or press Enter for all): ").strip()
    ...
    # Process players
    detailed_players = scraper.process_players(players_df, max_players)
    
    if detailed_players:
        # Save detailed data
        df_detailed = pd.DataFrame(detailed_players)
        csv_file = 'players_DETAILED.csv'
